...
- Report renderer with two formats, selectable via `--format` (`-f`): plain text (default — the compact `✗ Beacon … • code message` form with a one-line summary) and Markdown (beacon-grouped violations, a notices section, and a summary table).
- Command-line interface (`tracewright --rules <file> [--format text|markdown] <export.csv>`).
- Synthetic example rule sets and captures, plus a standalone `examples/beacon-emitter.html` beacon generator (with an on-page usage walkthrough) — all invented against public Adobe documentation, with no real-world data.
